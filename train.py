...
def validate(model, criterion, val_loader, iteration, writer):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            n_data += len(batch[0])
            text_padded, text_lengths, mel_padded, mel_lengths, duration_padded, pitch_padded, energy_padded = [x.cuda() for x in batch]
            mel_out, duration_out, durations, pitch_out, energy_out = model.module.outputs(text_padded,
                                                                     duration_padded,
                                                                     text_lengths,
                                                                     mel_lengths)
            mel_loss, duration_loss, pitch_loss, energy_loss= criterion((mel_out, duration_out, pitch_out, energy_out),
                                                (mel_padded, durations, pitch_padded, energy_padded),
                                                (text_lengths, mel_lengths))
            val_loss += (mel_loss+duration_loss+pitch_loss+energy_loss).item()*len(batch[0])
...
def main():
    ...
    while iteration < (hparams.train_steps*hparams.accumulation):
        for i, batch in enumerate(train_loader):
            text_padded, text_lengths, mel_padded, mel_lengths, duration_padded, pitch_padded, energy_padded = [
            ...
            if iteration%hparams.accumulation == 0:
                ...
                writer.add_scalar('duration_loss', duration_loss.item(),
                                  global_step=iteration//hparams.accumulation)
                writer.add_scalar('pitch_loss', pitch_loss.item(),
                                  global_step=iteration//hparams.accumulation)
                writer.add_scalar('energy_loss', energy_loss.item(),
                                  global_step=iteration//hparams.accumulation)
